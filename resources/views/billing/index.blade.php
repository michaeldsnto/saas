<x-app-layout>
    <x-slot name="header"><div><p class="text-sm uppercase tracking-[0.3em] text-cyan-300">Billing</p><h2 class="text-3xl font-semibold">Plans & Subscription</h2></div></x-slot>
    ...
    <section class="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <div class="rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-6">
            ...
            <h3 class="mt-3 text-3xl font-semibold">{{ $currentPlan?->name ?? 'No Active Plan' }}</h3>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        @foreach($plans as $plan)
            ...
            @endphp
            <form method="POST" action="{{ route('billing.checkout') }}" class="rounded-3xl border p-6 {{ $isCurrent ? 'border-cyan-400/40 bg-cyan-400/10' : 'border-white/10 bg-white/5' }}">
                @csrf
                ...
                <p class="mt-5 text-4xl font-semibold">Rp {{ number_format($plan->price, 0, ',', '.') }}</p>
                ...
                <x-primary-button class="mt-6">{{ $isCurrent ? 'Renew / Recreate Plan' : 'Choose Plan' }}</x-primary-button>
            </form>
        @endforeach
    </div>
